...
@app.route('/')
def index():
    x = open('mysite/textdata/post.txt', 'r')
    filer = x.readlines()
    x.close()
...
def myform():
    ...
    if form.validate_on_submit():
        ...
        x = open('mysite/textdata/post.txt', 'a')
        x.write('\n' + poster + ': ' + posted)
        x.close()
        return redirect('/')
...
def hangedman():
    ...
    x = open('mysite/textdata/hmdata.txt', 'r')
    hmdata = x.readlines()
    x.close()
    x = open('mysite/textdata/hmwords.txt', 'r')
    ...
    if formnew.validate_on_submit() and request.form['btn'] == 'New Game':
        hmwords = open('mysite/textdata/hmwords.txt', 'r')
        ...
        for i in wordlist:
            ...
            if formnew.gametype.data == l[0]:
                ...
                x = open('mysite/textdata/hmdata.txt', 'w+')
                x.write(newgame + '\n')
                x.close()
    ...
    if form.validate_on_submit() and request.form['btn'] == 'Submit':
        ...
        if hminput in hmdata[1]:
            return render_template('hangedman.html', form=form, word=word,
                                   tried=tried, missed=missed, alreadypick=True,
                                   category=category, ttl='Hangman')
        x = open('mysite/textdata/hmdata.txt', 'a')
        x.write(hminput)
        x.close()
        return redirect('/hangman')
...
class NHMForm(Form):
    gametype = RadioField(
        'gametype', choices=[('animals','Animals (easy)'),('food','Food (easy)'),
        ('technology','Technology'),('countries','Countries')],
        default = 'animals'
    )